...
def main():
    ...
    if 'vgg' in args.model:
        para = (para for name, para in model.named_parameters() if not ('0' not in name and 'conv' in name))
        conv_para = (para for name, para in model.named_parameters() if ('0' not in name and 'conv' in name))
    else:
        para = (para for name, para in model.named_parameters() if not ('layer' in name and 'conv' in name))
        conv_para = (para for name, para in model.named_parameters() if ('layer' in name and 'conv' in name))
    if args.optimizer == 'sgd':
        optimizer = torch.optim.SGD([{'params':para,'initial_lr':args.lr},{'params':conv_para, 'initial_lr':args.lr, 'weight_decay':0.}],
                                    lr=args.lr, 
                                    momentum=args.momentum, 
                                    weight_decay=args.weight_decay)
    elif args.optimizer == 'adam':
        optimizer = torch.optim.Adam([{'params':para,'initial_lr':args.lr},{'params':conv_para, 'initial_lr':args.lr, 'weight_decay':0.}],
                                    lr=args.lr, 
                                    weight_decay=args.weight_decay) 
    else: 
    ...
    if 'vgg' in args.model:
        for name, module in model.named_modules():
            if isinstance(module,nn.Conv2d) and '0' not in name:
